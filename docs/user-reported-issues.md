...
- Existing pieces to build on: the daytrips page already has a `copyToClipboard`
  helper; the Slack polls (`src/lib/server/slackPlaygroupPoll.ts`,
  `slackFeedingPoll.ts`) import playgroup and feeding messages — this feature is the outbound counterpart
