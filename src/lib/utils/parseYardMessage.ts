...
const YARD_TIME = /\byard\s*(?:time|tine|tim)\b|\bin\s+the\s+yards?\b|\bgot\s+out\s+today\b/i;
 ...
 */
const NOT_A_REPORT =
	/\bdo\s*n'?t\s+leave\b|\bdo\s+not\s+leave\b|\bwhen\s+you\b|\bmake\s+sure\b|\bneeds?\s+to\b|\bwill\s+have\s+to\b|\bshould\b|\bcan\s+be\b|\bis\s+terrified\b/i;

/** The whole shelter at once. */
const ALL_DOGS =
	/\ball\s+(?:the\s+)?(?:healthy\s+|able\s+|available\s+)?dogs?\b|\beveryone\b|\ball\s+of\s+(?:the\s+)?dogs?\b/i;
...
export function parseYardMessage(text: string, knownDogNames: string[] = []): ParsedYardMessage {
	const empty = { dogNames: [], allDogs: false, durationMinutes: null, negated: false };

	const yard = YARD_TIME.exec(text);
	if (!yard) return empty;
	...
	// A blanket covers the shelter, so no name list is needed or expected.
	if (ALL_DOGS.test(text.slice(0, yard.index + yard[0].length))) {
		return { dogNames: [], allDogs: true, durationMinutes, negated: false };
	}
	...
	const before = text.slice(0, yard.index);
	...
	return { dogNames, allDogs: false, durationMinutes, negated: false };
}
