...
import { backfillSlackBaths } from '$lib/server/slackBathBackfill';

// Paging months of Slack history can take a while.
export const config = { maxDuration: 60 };

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

/** Admin-only: backfills baths reported in #dog-staff. Dry run unless told otherwise. */
export async function POST({ request }: RequestEvent) {
	...
	const body = (await request.json().catch(() => ({}))) as { since?: string; dryRun?: boolean; keep?: unknown };
	...
	return json(await backfillSlackBaths(since, body.dryRun !== false, keep));
}
